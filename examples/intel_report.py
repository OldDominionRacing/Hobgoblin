...
It shows the full deterministic ("goblin-first") pipeline on one paragraph:
  ...
  3. military unit recognition (echelon + designation)
...
def main():
    ...
    for e in ents:
        ...
        if e["mil_unit"]:
            m = e["mil_unit"]
            line += f"   unit<{m['echelon']} {m['designation']} ({m['designation_form']})>"
    ...
    for u in detect_units(REPORT):
        branch = f" {u['branch']}" if u["branch"] else ""
        print(f"  {u['text']!r:24} -> {u['echelon']} #{u['designation']}{branch}")
